...
if(!function_exists("anti_injection")) {
	function anti_injection($obj) {
		if(strnatcmp(phpversion(),'5.6.0') <= 0) {
			$obj = @ preg_replace(sql_regcase("/(from|select|insert|delete|where|drop table|show tables|#|\*|--|\\\\)/"),"",$obj);
			$obj = trim($obj);
			$obj = strip_tags($obj);
			$obj = addslashes($obj);
		}
...
function busca_produtos($inputCampo, $campoId, $targetParent, $tituloBusca, $tituloBanco, $javascript, $tabelas, $concatCampos, $where, $nome_campo='') { 
	...
	$onClick = $targetParent."abreJanelaJquery('select-busca-produtos.php?$gets', 'Busca de ".$tituloBusca."', '', '450px', '200', ".$targetParent."rand(1,9999))";
	?>
    <script>
	var tempoDados = '';
	...
	function pegaDadosBGen<?=$inputCampo;?>() {
		if(document.getElementById('<?=$inputCampo;?>').value != document.getElementById('digitadoBusGen<?=$inputCampo;?>').value && document.getElementById('<?=$inputCampo;?>').value != '') {
			document.getElementById('enviaBuscaGenerica<?=$inputCampo;?>').src='select-busca-produtos.php?busca=***&id=' + document.getElementById('<?=$inputCampo;?>').value + '&<?=$gets;?>';
...
function buscaGenericad($inputCampo, $campoId, $targetParent, $tituloBusca, $tituloBanco, $javascript, $tabelas, $concatCampos, $where, $nome_campo='') { 
	$_SESSION[javascript_.$inputCampo] = $javascript;
	$gets = "targetParent=".$targetParent."&inputCampo=".$inputCampo."&tituloBanco=".$tituloBanco."&tabelas=".$tabelas."&campoId=".$campoId."&concatCampos=".$concatCampos."&where=".urlencode($where);
...
function url($nome) {
	return(strtolower(str_replace(' ', '-', retira_acentos(trim($nome)))));
}

function retira_acentos($texto) {

        $array1 = array("á", "à", "â", "ã", "ä", "é", "è", "ê", "ë", "í", "ì", "î", "ï", "ó", "ò", "ô", "õ", "ö", "ú", "ù", "û", "ü", "ç", "Á", "À", "Â", "Ã", "Ä", "É", "È", "Ê", "Ë", "Í", "Ì", "Î", "Ï", "Ó", "Ò", "Ô", "Õ", "Ö", "Ú", "Ù", "Û", "Ü", "Ç", ' ', 'Â', 'Ã', ',', '/', '"', '.', '(', ')', '+', ' ');

        $array2 = array("a", "a", "a", "a", "a", "e", "e", "e", "e", "i", "i", "i", "i", "o", "o", "o", "o", "o", "u", "u", "u", "u", "c", "A", "A", "A", "A", "A", "E", "E", "E", "E", "I", "I", "I", "I", "O", "O", "O", "O", "O", "U", "U", "U", "U", "C", '_', 'A', 'A', '', '_', '', '', '', '', '_', '');
				
        $retorno = str_replace($array1, $array2, $texto);
		return(substr($retorno,0,40));
    }
...
function upload($campo, $pasta, $array) {
	...
	if($array == 'N' and $array != '0') {
		...
		if($_FILES[$campo]["name"][$array]) {
			$file = $_FILES[$campo];
			$file["name"][$array];
			$ext = strtolower(substr($file["name"][$array],strrpos($file["name"][$array],".")));
